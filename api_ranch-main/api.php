    ...
    if ($action === 'login') {
        ...
        try {
            ...
            if ($user && $user['mdp'] === $password) {
                ...
                $stmt = $pdo->prepare("SELECT * FROM cavalier WHERE email = :email");
                $stmt->execute(['email' => $email]);
                $cavalier = $stmt->fetch(PDO::FETCH_ASSOC);

                if ($cavalier && $cavalier['mdpcav'] === $password) {
                    echo json_encode([
                        "success" => true,
                        "message" => "Connexion réussie",
                        "user" => [
                            "id" => $cavalier['idcavalier'],
                            "nom" => $cavalier['nomcav'],
                            "prenom" => $cavalier['prenomcav'],
        ...
        try {
            $stmt = $pdo->prepare("
                SELECT c.nomcav as nom, c.prenomcav as prenom
                FROM inscrit i
                JOIN cavalier c ON i.idcavalier = c.idcavalier
