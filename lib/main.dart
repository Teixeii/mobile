...
class _CoursesPageState extends State<CoursesPage> {
  ...
  Future<List<Cours>> fetchCours() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Cours> cours = data.map((json) => Cours.fromJson(json)).toList();
      _joursDisponibles.addAll(
        cours.map((c) => c.jour).toSet().where((j) => j.isNotEmpty),
      );
      _applyFilters(cours);
      return cours;
    } else {
  ...
  void _applyFilters(List<Cours> cours) {
    setState(() {
      _filteredCours =
          cours.where((cours) {
            final lib = cours.libelle.toLowerCase();
            final jour = cours.jour;
            final hd = cours.heureDebut;
            final hf = cours.heureFin;
            ...
            final matchStart =
                ...
                TimeOfDay.fromDateTime(
                      DateTime.parse('2023-01-01 $hd'),
                    ).compareTo(_selectedStartTime!) >=
                    0;
            final matchEnd =
                _selectedEndTime == null ||
                TimeOfDay.fromDateTime(
                      DateTime.parse('2023-01-01 $hf'),
                    ).compareTo(_selectedEndTime!) <=
                    0;
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items:
                        _joursDisponibles
                            .map(
                              (j) => DropdownMenuItem(child: Text(j), value: j),
                            )
                            .toList(),
                    onChanged: (val) {
                      _selectedDay = val!;
